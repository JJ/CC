Hito 0: Descripción del proyecto usando correctamente `git` y GitHub
=====================================
...
> documentadas y entenderlas correctamente. Hacerlo es un prerrequisito para
...
de objetivos tendrá el nombre del usuario de GitHub (siempre en minúsculas), y estará en
...
aplicación irá evolucionando con los diferentes hitos. En cada hito el
fichero README.md describirá el estado del proyecto actual, y se
sacará a otros ficheros (enlazándolos) la documentación adicional que
haya podido necesitarse para la corrección de otros hitos.
...
1. 4 puntos: Repositorio individual creado y entregado
  correctamente, con todos los ficheros que se solicitan, y el
  contenido correcto en cada uno de los ficheros. Si se incumplen alguna de las buenas prácticas en la
  entrega puede haber penalización, aunque no por envíos sucesivos
  dentro del mismo PR.
2. 4 puntos: Presencia de todos los ficheros de documentatión necesarios y entregados
  correctamente, con la configuración de usuario y repositorio correcta, incluyendo
  la definición de hitos y órdenes de trabajo (*issues*) creadas y
  cerradas correctamente y la configuración del entorno de trabajo.

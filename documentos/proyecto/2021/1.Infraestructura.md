...
Haber entregado el anterior hito.
...
* [HUx] Como *miembro de la familia* (rol), necesito poder añadir
      (actividad) productos (datos) a la lista de la compra.

Pero también la siguiente:

* [HUy] Como *administrador de la aplicación*, necesito analizar
...
mismo, y que sea desplegable en la nube. Esas historias de usuario,
además, deberán estar acompañadas de otras que definan claramente
tanto los roles como cada una de las entidades que se mencionan. En
particular, no se podrá avanzar con HUx hasta que el rol de "miembro
de la familia" esté bien definido (y en particular si va a ser un
usuario genérico o va a tener también algun tipo de objeto que lo
represente en la aplicación), y las entidades producto y lista de la
compra estén también bien definidas. En particular, en el cso "lista
de la compra" sería conveniente crear un issue adicional:
...
> En todas estas cosas conviene tener siempre en mente el problema
> inicial que se ha planteado. Por ejemplo, en el caso de la lista de
> la compra lo que necesitábamos, por un lado, es una herramienta útil
> para una familia, pero por otro lado que se pueda analizar y extraer
> datos de ella. Por eso habría que considerar cosas adicionales como
> que, en mi caso, hay veces que se quedan cosas en la lista de la
> compra porque no hay existencias (o se te han olvidado), así que
> debería ser posible crear una lista de la compra a partir de la
> anterior. Eso implica, a su vez, una entidad (agregado) que sea
> capaz de almacenar un histórico de listas e la compra (lo que sería
> un issue, eso no es una HU). Y así sucesivamente. Como se explica un
> poco más adelante, las HUs no es algo que tenga que estar todo desde
> el principio. Generalmente se genera una única HU, que puede ser el
> objetivo final, y a partir de ahí van surgiendo otras HUs que
> definan claramente todo lo que se describe ahí, issues que pidan
> resolver el problema de la implementación, lo que a su vez dará
> lugar a otras HUs más adelante.
...
* A continuación tiene que haber HUs que establezcan las diferentes
  ...
  simplemente
  el [primer capítulo](../temas/Arquitecturas_para_la_nube) donde
...
Las HUs van a guiar el desarrollo del código (y la
infraestructura-como-código) durante toda la asignatura. Es mejor no
hacer un montón ahora que luego haya que reescribir, sino ir añadiendo
poco a poco HUs según se vayan necesitando.
...
> añadir un atributio (grimorio) y un método (almacenar encantamiento)
...
[colección de consejos](https://www.romanpichler.com/blog/1.md0-tips-writing-good-user-stories/),
...
### Los issues avanzan siempre HUs
...
ejemplo). En todo caso, el issue lo que declarea es el problema que
...
El plan agrupará HUs en épicas, que se
podrán
[transformar en milestones](https://gitlab.com/gitlab-org/gitlab/-/issues/6.md222)
del proyecto. Realmente
corresponden
[a conceptos diferentes](https://blog.zenhub.com/how-to-use-epics-and-milestones/),
pero no es difícil convertir unas (concepto metodológico) en otra
(funcionalidad que ofrece GitHub). Aquí, sin embargo [agrupa varias
épicas en un milestone](https://clubhouse.io/blog/how-we-use-milestones-epics-product-management-clubhouse/).
...
recurrir a un test, que crea un overhead sobre el código.
...
* El README tendrá que describir la arquitectura que va a seguir el
  ...
  el [primer capítulo](../temas/Arquitecturas_para_la_nube).
...
1. 3 puntos: Descripción correcta y justificada de la arquitectura,
  ...
  del [primer capítulo](../temas/Arquitecturas_para_la_nube).
